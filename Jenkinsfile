    ...
    stages {
        ...
        stage('Run tests') {
			agent {
				docker {
					...
					args '-p 3000:3000 --network ${NETWORK_NAME} -v "C:/project/allureResults":/var/jenkins_home/workspace/UI_Tests_With_Allure@2/Tests_For_TestInfrastructure_Course/bin/Debug/netcoreapp2.1/allure-results' 
				}
			}
            steps {
		...
		stage('Reports') {
			steps {
				...
				//sh "cp -a ./Tests_For_TestInfrastructure_Course/bin/Debug/netcoreapp2.1/allure-results/. /var/jenkins_home/workspace/UI_Tests_With_Allure/allure-report/"
				script {
						allure([
