    ...
    stages {
        ...
        stage('Test') {
            ...
            steps {
				...
				sh "sed -i 's/ToDoApplicationUrl_Value/${FRONTEND_URL}' Tests_For_TestInfrastructure_Course/config/docker.runsettings"
				sh 'cat Tests_For_TestInfrastructure_Course/config/docker.runsettings'
				sh "sed -i 's/SeleniumGridUrl_Value/${BROWSER_URL}' Tests_For_TestInfrastructure_Course/config/docker.runsettings"
				sh 'cat Tests_For_TestInfrastructure_Course/config/docker.runsettings'
